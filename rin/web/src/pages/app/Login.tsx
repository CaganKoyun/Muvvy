  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } =
      mode === 'login'
        ...
        : await db.signUp(email.trim(), password, 'consumer', email.trim());
    setBusy(false);
    ...
    <div className="flex min-h-screen items-center justify-center p-6">
      <Card className="w-full max-w-sm p-8">
        ...
        <div className="mt-6 flex items-center gap-2 rounded-xl bg-spark/5 p-3">
          <SparkMark /><div className="text-sm font-medium text-ink">Continue with Spark</div>
        </div>
        <p className="mt-3 text-sm text-slate-500">One identity for every store. Seed shopper: <code className="text-xs">ahmet@example.com / password123</code></p>
        <div className="mt-4 grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={() => db.oauth('google')}>Google</Button>
          <Button variant="outline" onClick={() => db.oauth('apple')}>Apple</Button>
        </div>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <Button className="w-full" disabled={busy}>{busy ? <Spinner /> : mode === 'login' ? 'Continue' : 'Create identity'}</Button>
        </form>
