...
export default function Branches() {
  ...
  async function load() { setBranches(await db.branches()); setLoading(false); }
  ...
  async function add(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await db.addBranch({ name: form.name, code: form.code.toUpperCase(), city: form.city || undefined });
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_320px]">
        <Card>
          {loading ? <div className="flex justify-center py-12"><Spinner /></div> :
            branches.length === 0 ? <p className="p-6 text-sm text-slate-400">No branches yet.</p> : (
            <ul className="divide-y divide-slate-100">
              {branches.map((b) => (
                <li key={b.id} className="flex items-center justify-between px-5 py-3">
                  <div><div className="font-medium text-ink">{b.name}</div><div className="text-xs text-slate-500">{b.city || '—'}</div></div>
                  <Badge>{b.code}</Badge>
                </li>
              ))}
            </ul>
